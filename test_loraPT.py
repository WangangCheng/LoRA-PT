...
#from models.AttUnet_model import AttentionUNet_4pools
...
#from models.TransUnet_Fs import Model
#from models.Unet_channel_ex.Unet3D_model import UNet_4pools
...
parser.add_argument('--valid_dir', default='UPENN/', type=str)

parser.add_argument('--valid_file', default='test.txt', type=str)

parser.add_argument('--output_dir', default='loraPT_output', type=str)
...
parser.add_argument('--test_date', default='2025-12-04', type=str)
...
def main():
    ...
    if args.lora_dim > 0:
        print("Start converting to LoRA-PT...")
        model = convert_linear_layer_to_lorapt(model, 'vit', args.lora_dim, init_from_tsvd=False)
        print("Finished LoRA-PT conversion.")
        
    model = model.cuda()
    
    load_file = os.path.join(os.path.abspath(os.path.dirname(__file__)),
                             'checkpoint', args.experiment+args.test_date,'r' + str(args.lora_dim), args.test_file)
    print(load_file)
    ...
    if os.path.exists(load_file):
        ...
        print('Successfully load checkpoint {}'.format(os.path.join(args.experiment+args.test_date,'r' + str(args.lora_dim), args.test_file)))
    else:
    ...
    submission = os.path.join(os.path.abspath(os.path.dirname(__file__)), args.output_dir,'r' + str(args.lora_dim),
                              args.submission, args.experiment+args.test_date)
    visual = os.path.join(os.path.abspath(os.path.dirname(__file__)), args.output_dir,'r' + str(args.lora_dim),
